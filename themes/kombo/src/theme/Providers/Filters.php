    ...
    static function TheContent($content)
    {
        if ($html = HtmlDomParser::str_get_html($content)) {
            foreach ($html->find("img") as $img) {
                ...
                if (strpos($classes, "img-fluid") === false && strpos($classes, "no-fluid") === false) {
                    $img->setAttribute("class", trim($classes . " img-fluid"));
                }

                if (!$img->hasAttribute("data-src") && strpos($classes, "lazy") === false) {
                    $src = $img->getAttribute("src");
                    $img->setAttribute("src", "");
                    $img->setAttribute("data-src", $src);
                    $img->setAttribute("class", trim($classes . " lazy"));
                }
            ...
            foreach ($html->find("iframe") as $iframe) {
                ...
                if (strpos($classes, "no-fluid") === false) {
                    $iframe->outertext = "<div class=\"ratio ratio-16x9\">" . $iframe->outertext . "</div>";
                }

                if (!$iframe->hasAttribute("data-src") && strpos($classes, "lazy") === false) {
                    $src = $iframe->getAttribute("src");
                    $iframe->setAttribute("src", "");
                    $iframe->setAttribute("data-src", $src);
                    $iframe->setAttribute("class", trim($classes . " lazy"));
                }
            }
